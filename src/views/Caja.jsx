...
import { fmt, fmtDate, fmtDateTime, todayStr, daysUntil } from '../utils/helpers'
...
export default function Caja() {
  ...
  const [kpis, setKpis] = useState({ saldoCierre: 0, ventas: 0, ingExtra: 0, egresos: 0, saldoActual: 0, ventasBanco: 0, ventasEfectivo: 0, egresosBanco: 0, egresosEfectivo: 0, saldoBanco: 0, saldoEfectivo: 0 })
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      // Fallback: si no tiene timestamp, usar fecha
      if (!cierre) {
        const fallback = await getDocs(query(collection(db, 'cierres_caja'), orderBy('fecha', 'desc'), limit(1)))
        fallback.forEach(d => { cierre = { id: d.id, ...d.data() } })
      }
      setLastCierre(cierre)

      // 2. Alerta días sin cierre
      const hoy = todayStr()
      const ayer = new Date(Date.now() - 86400000).toISOString().split('T')[0]
      if (!cierre) {
        setAlertMsg('No hay cierres registrados. Ingresá el saldo inicial haciendo un cierre de caja.')
      ...
      // 3. Movimientos DESDE el timestamp exacto del cierre (Opción A)
      ...
      const [orders, egrSnap, extraSnap] = await Promise.all([
        fetchOrders(desdeTimestamp, ahora, 'completed,processing').catch(() => []),
      ...
      // 4. Compromisos para proyección
      const compSnap = await getDocs(collection(db, 'compromisos'))
      const comps = []
      compSnap.forEach(d => comps.push({ id: d.id, ...d.data() }))
  ...
  const calcProyeccion = async (saldoActual, comps) => {
    ...
    const pagadoPorComp = {}
    sueldoSnap.forEach(d => {
      const x = d.data()
      if (x.origen_compromiso) pagadoPorComp[x.origen_compromiso] = (pagadoPorComp[x.origen_compromiso] || 0) + parseFloat(x.monto || 0)
    })
    ...
    // AGIP
    const vencAGIP = new Date(now.getFullYear(), now.getMonth(), 10)
    if (vencAGIP >= now) {
      try {
        const mesAnt = new Date(now.getFullYear(), now.getMonth() - 1, 1)
        const mesAntFin = new Date(now.getFullYear(), now.getMonth(), 0, 23, 59, 59)
        const ordBase = await fetchOrders(mesAnt, mesAntFin, 'completed,processing')
        const base = ordBase.reduce((s, o) => s + parseFloat(o.total || 0), 0)
        if (base > 0) pendientes.push({ nombre: 'AGIP — Ingresos Brutos', monto: base * 0.04, categoria: 'impuestos', fecha: vencAGIP.toISOString().split('T')[0] })
      } catch {}
    }

    const totalPendiente = pendientes.reduce((s, p) => s + p.monto, 0)
    setProyeccion({ saldoActual, totalPendiente, saldoProyectado: saldoActual - totalPendiente, pendientes })
  }
  ...
  const guardarCierre = async () => {
    ...
    if (!existing.empty) {
      ...
      for (const d of existing.docs) await deleteDoc(doc(db, 'cierres_caja', d.id))
    }
    await addDoc(collection(db, 'cierres_caja'), {
      fecha: cierreForm.fecha,
      saldo_final: saldo,
      saldo_banco: parseFloat(cierreForm.saldo_banco) || 0,
      saldo_efectivo: parseFloat(cierreForm.saldo_efectivo) || 0,
  ...
  // ── Ingreso extra ────────────────────────────────────────
  const openNuevoExtra = () => {
    setEditExtra(null)
    setExtraForm({ fecha: todayStr(), monto: '', tipo: 'credito', descripcion: '' })
    setShowExtra(true)
  }
  ...
  const deleteExtra = async (id) => {
    ...
    await deleteDoc(doc(db, 'ingresos_extra', id))
  ...
  const deleteCierre = async (id) => {
    ...
    await deleteDoc(doc(db, 'cierres_caja', id))
    ...
    <div className="view">
      ...
      <div className="view-header-row">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button className="btn btn-secondary" onClick={openNuevoExtra}>
      ...
      <div className="cards-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 12 }}>
        <div className="stat-card card-blue">
          <div className="card-label">Saldo último cierre</div>
          <div className="card-value">{fmt(kpis.saldoCierre)}</div>
          <div className="card-sub">{lastCierre ? 'al ' + fmtDate(lastCierre.fecha) + (lastCierre.timestamp ? ' ' + new Date(lastCierre.timestamp).toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'}) : '') : 'sin cierre previo'}</div>
        </div>
        <div className="stat-card card-green">
          <div className="card-label">+ Ventas desde cierre</div>
          <div className="card-value">{fmt(kpis.ventas)}</div>
          <div className="card-sub">{kpis.ordenes} órdenes</div>
        </div>
        <div className="stat-card card-red">
          <div className="card-label">− Egresos desde cierre</div>
          <div className="card-value">{fmt(kpis.egresos)}</div>
        </div>
        <div className="stat-card" style={{ borderLeft: '3px solid var(--blue)', background: 'linear-gradient(135deg,#eff3ff,#fff)' }}>
          <div className="card-label">Saldo actual estimado</div>
          <div className="card-value" style={{ fontSize: 22, color: kpis.saldoActual >= 0 ? 'var(--blue)' : 'var(--danger)' }}>
            {fmt(kpis.saldoActual)}
          </div>
          <div className="card-sub">en cuenta ahora</div>
        </div>
      </div>

      {/* KPIs — fila 2: desglose banco / efectivo */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 10, padding: '16px 20px', boxShadow: 'var(--shadow)', borderLeft: '4px solid #2e509e' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
            <span>+ Ventas transferencia</span><span style={{ color: 'var(--success)', fontWeight: 600 }}>{fmt(kpis.ventasBanco)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
            <span>+ Ingresos extra</span><span style={{ color: 'var(--success)', fontWeight: 600 }}>{fmt(kpis.ingExtra)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
            <span>− Egresos banco</span><span style={{ color: 'var(--danger)', fontWeight: 600 }}>{fmt(kpis.egresosBanco)}</span>
          </div>
        ...
        <div style={{ background: '#fff', borderRadius: 10, padding: '16px 20px', boxShadow: 'var(--shadow)', borderLeft: '4px solid #FF6B00' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
            <span>+ Ventas efectivo</span><span style={{ color: 'var(--success)', fontWeight: 600 }}>{fmt(kpis.ventasEfectivo)}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-muted)', marginBottom: 8, marginTop: 20 }}>
            <span>− Egresos efectivo</span><span style={{ color: 'var(--danger)', fontWeight: 600 }}>{fmt(kpis.egresosEfectivo)}</span>
          </div>
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div className="chart-card">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Proyección al {fmtDate(new Date(new Date().getFullYear(), new Date().getMonth() + 1, 0).toISOString().split('T')[0])}
            </span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            <div style={{ textAlign: 'center', padding: 16, background: '#f8fafc', borderRadius: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Saldo actual</div>
              <div style={{ fontFamily: 'var(--font-head)', fontSize: 20, fontWeight: 700 }}>{fmt(proyeccion.saldoActual)}</div>
            </div>
            <div style={{ textAlign: 'center', padding: 16, background: '#fff3e8', borderRadius: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--orange)', marginBottom: 6 }}>− Compromisos pendientes</div>
              <div style={{ fontFamily: 'var(--font-head)', fontSize: 20, fontWeight: 700, color: 'var(--orange)' }}>{fmt(proyeccion.totalPendiente)}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{proyeccion.pendientes.length} compromisos</div>
            </div>
            <div style={{ textAlign: 'center', padding: 16, background: proyeccion.saldoProyectado >= 0 ? '#ecfdf5' : '#fef2f2', borderRadius: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>= Saldo proyectado</div>
              <div style={{ fontFamily: 'var(--font-head)', fontSize: 22, fontWeight: 800, color: proyeccion.saldoProyectado >= 0 ? 'var(--success)' : 'var(--danger)' }}>
                {fmt(proyeccion.saldoProyectado)}
              </div>
            </div>
          </div>
          {proyeccion.pendientes.length > 0 && (
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12, marginTop: 14 }}>
              <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>
                Detalle de compromisos pendientes
              </div>
              {proyeccion.pendientes.sort((a, b) => a.fecha.localeCompare(b.fecha)).map((p, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span className="cat-dot" style={{ background: getCat(p.categoria).color }} />
                    {p.nombre}
      ...
      <div className="table-card">
        <div className="table-card-header">
          ...
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Créditos, transferencias y otros desde el último cierre</span>
        </div>
        {extraDocs.length === 0
          ? <div className="empty-state"><p>Sin ingresos extra desde el último cierre</p></div>
          : <table>
              <thead><tr><th>Fecha</th><th>Tipo</th><th>Descripción</th><th className="text-right">Monto</th><th className="text-center">Acciones</th></tr></thead>
              <tbody>
                {extraDocs.map(x => (
                  <tr key={x.id}>
                    <td>{fmtDate(x.fecha)}</td>
                    <td><span className="badge badge-green">{tipoLabel[x.tipo] || x.tipo}</span></td>
                    <td>{x.descripcion || '-'}</td>
                    <td className="text-right"><strong style={{ color: 'var(--success)' }}>{fmt(x.monto)}</strong></td>
      ...
      <div className="table-card">
        ...
        {historial.length === 0
          ? <div className="empty-state"><p>Sin cierres registrados</p></div>
          : <table>
              <thead><tr><th>Fecha</th><th>Hora</th><th className="text-right">Total</th><th className="text-right">Banco</th><th className="text-right">Efectivo</th><th>Notas</th><th className="text-center">Acciones</th></tr></thead>
              <tbody>
                {historial.map(c => (
                  <tr key={c.id}>
                    <td><strong>{fmtDate(c.fecha)}</strong></td>
                    <td style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      {c.timestamp ? new Date(c.timestamp).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' }) : '-'}
                    </td>
                    ...
                    <td className="text-center">
                      <button className="btn btn-danger btn-sm" onClick={() => deleteCierre(c.id)}>×</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setShowCierre(false)}>
          <div className="modal" style={{ width: 420 }}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowCierre(false)}>×</button>
            </div>
            <div style={{ background: '#f0f4f8', borderRadius: 8, padding: 14, marginBottom: 16, fontSize: 13 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                <span style={{ color: 'var(--text-muted)' }}>Saldo calculado:</span>
                <strong>{fmt(saldoCalc)}</strong>
              </div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                Ajustá si hay diferencia con el saldo real en cuenta.
              </div>
            </div>
            <div className="field">
              <label>Saldo total en cuenta ($)</label>
              <input type="number" value={cierreForm.saldo} onChange={e => setCierreForm(f => ({ ...f, saldo: e.target.value }))} />
            </div>
            <div className="field-row">
              <div className="field">
                <label>Del cual, en banco ($)</label>
                <input type="number" value={cierreForm.saldo_banco} onChange={e => setCierreForm(f => ({ ...f, saldo_banco: e.target.value }))} placeholder="0" />
              </div>
              <div className="field">
                <label>Del cual, en efectivo ($)</label>
                <input type="number" value={cierreForm.saldo_efectivo} onChange={e => setCierreForm(f => ({ ...f, saldo_efectivo: e.target.value }))} placeholder="0" />
              </div>
            </div>
            <div className="field">
              <label>Fecha del cierre</label>
              <input type="date" value={cierreForm.fecha} onChange={e => setCierreForm(f => ({ ...f, fecha: e.target.value }))} />
            </div>
            <div className="field">
              <label>Notas (opcional)</label>
              <textarea value={cierreForm.notas} onChange={e => setCierreForm(f => ({ ...f, notas: e.target.value }))} placeholder="Observaciones..." />
            </div>
      ...
      {showExtra && (
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setShowExtra(false)}>
          <div className="modal" style={{ width: 420 }}>
            <div className="modal-header">
              <h3>{editExtra ? 'Editar Ingreso Extra' : 'Nuevo Ingreso Extra'}</h3>
              <button className="modal-close" onClick={() => setShowExtra(false)}>×</button>
            </div>
            <div className="field-row">
              <div className="field">
                <label>Fecha</label>
                <input type="date" value={extraForm.fecha} onChange={e => setExtraForm(f => ({ ...f, fecha: e.target.value }))} />
              </div>
              <div className="field">
                <label>Monto ($)</label>
                <input type="number" value={extraForm.monto} onChange={e => setExtraForm(f => ({ ...f, monto: e.target.value }))} placeholder="0" />
              </div>
            </div>
            <div className="field">
              <label>Tipo</label>
              <select value={extraForm.tipo} onChange={e => setExtraForm(f => ({ ...f, tipo: e.target.value }))}>
            ...
            <div className="field">
              ...
              <textarea value={extraForm.descripcion} onChange={e => setExtraForm(f => ({ ...f, descripcion: e.target.value }))} placeholder="Ej: Crédito banco BBVA..." />
            </div>
            <div className="modal-footer">
...
// Exported migration helper — run once from Config or directly
export async function migrarCierresSinTimestamp(db, toast) {
